...
async function verifyCommandsCount() {
  ...
  if (registeredCommandsCount !== expectedCommandsCount) {
    ...
    console.log(
      `${colors.yellow}[ DETAILS ]${colors.reset} ${colors.red}Current Commands: ${colors.reset}${registeredCommandsCount}`,
    )
    console.log(
      `${colors.yellow}[ DETAILS ]${colors.reset} ${colors.red}Expected Commands: ${colors.reset}${expectedCommandsCount}`,
    )
    console.log(
      `${colors.yellow}[ STATUS  ]${colors.reset} ${colors.red}Action Required: Please verify command integrity${colors.reset}`,
    )
  } else {
    console.log(
      `${colors.cyan}[ COMMANDS ]${colors.reset} ${colors.green}Command Count: ${registeredCommandsCount} ✓${colors.reset}`,
    )
  ...
  if (interaction.isButton() && interaction.customId.startsWith("open_application_modal_")) {
    ...
    if (!userId) {
      return interaction.followUp({
        content: "❌ Could not find the user who submitted the application.",
        ephemeral: true,
      })
    }
    ...
    try {
      ...
      interaction.followUp({
        content: `✅ The user has been notified that their application was **${status}**.`,
        ephemeral: true,
      })
    } catch (error) {
