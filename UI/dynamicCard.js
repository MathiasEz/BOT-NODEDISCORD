...
async function dynamicCard({ thumbnailURL, songTitle, songArtist, trackRequester, fontPath }) {
  const cardWidth = 800
  ...
  const randomBgPath = data.backgroundImages[Math.floor(Math.random() * data.backgroundImages.length)]
  ...
  ctx.arcTo(cardWidth - padding, padding, cardWidth - padding, padding + 20, 20)
  ctx.arcTo(cardWidth - padding, cardHeight - padding, cardWidth - padding - 20, cardHeight - padding, 20)
  ctx.arcTo(
  ...
  ctx.arcTo(cardWidth - thumbnailSize - padding, padding, cardWidth - thumbnailSize - padding + 20, padding, 20)
  ctx.closePath()
  ctx.clip()
  ctx.drawImage(thumbnailImage, cardWidth - thumbnailSize - padding, padding, thumbnailSize, thumbnailSize)
  ctx.restore()
  ...
  ctx.fillText(`Requested by: ${trackRequester}`, padding + 10, cardHeight - padding - 30)
