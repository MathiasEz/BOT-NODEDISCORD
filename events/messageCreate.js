    ...
    if (countingData && countingData.channelId === channelId && countingData.status) {
      ...
      if (!/^\d+$/.test(content)) {
        ...
        return message.channel
          .send(`${message.author}, please only send numbers!`)
          .then((msg) => setTimeout(() => msg.delete(), 3000))
      }

      const userNumber = Number.parseInt(content, 10)
      if (userNumber !== expectedCount) {
        await message.delete()
        return message.channel
          .send(`${message.author}, please follow the correct sequence! Next number should be **${expectedCount}**.`)
          .then((msg) => setTimeout(() => msg.delete(), 3000))
      }
        ...
        if (level > oldLevel) {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              ...
              {
                name: "💫 XP Gained This Week",
                value: `**${(await getUserData(message.author.id)).weeklyXp || 0} XP**`,
                inline: true,
              },
              { name: "📈 Total XP", value: `**${xp} XP**`, inline: true },
              {
                name: "✨ XP to Next Level",
                value: `**${Math.ceil(Math.pow((level + 1) / 0.1, 2) - xp)} XP**`,
                inline: true,
              },
            )
          ...
          if (logChannel) {
            await logChannel.send({
              content: `${message.author}, you leveled up to **level ${level}!** 🎉`,
              embeds: [embed],
            })
          } else {
