...
async function fetchLatestVideos(client) {
  ...
  for (const config of configs) {
    ...
    try {
      ...
      if (channel) {
        const thumbnailUrl = latestVideo.snippet.thumbnails?.high?.url || FALLBACK_THUMBNAIL
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Check out the new Video : [${latestVideo.snippet.title}](https://www.youtube.com/watch?v=${videoId})`,
          )
        ...
        const mentionText =
          mentionRoles && mentionRoles.length > 0 ? mentionRoles.map((roleId) => `<@&${roleId}>`).join(" ") : ""
