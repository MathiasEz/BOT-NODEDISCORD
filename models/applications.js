...
async function addQuestion(guildId, appName, question) {
  await applicationCollection.updateOne({ guildId, appName }, { $push: { questions: question } })
}
...
async function removeQuestion(guildId, appName, questionIndex) {
  ...
  await applicationCollection.updateOne({ guildId, appName }, { $set: { questions: app.questions } })
}
