  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) => option.setName("role1").setDescription("The first role ID").setRequired(true))
        .addStringOption((option) =>
          option.setName("label1").setDescription("The first button label").setRequired(true),
        )
        .addStringOption((option) => option.setName("role2").setDescription("The second role ID").setRequired(false))
        .addStringOption((option) =>
          option.setName("label2").setDescription("The second button label").setRequired(false),
        )
        .addStringOption((option) => option.setName("role3").setDescription("The third role ID").setRequired(false))
        .addStringOption((option) =>
          option.setName("label3").setDescription("The third button label").setRequired(false),
        )
        .addStringOption((option) => option.setName("role4").setDescription("The fourth role ID").setRequired(false))
        .addStringOption((option) =>
          option.setName("label4").setDescription("The fourth button label").setRequired(false),
        )
        .addStringOption((option) => option.setName("role5").setDescription("The fifth role ID").setRequired(false))
        .addStringOption((option) =>
          option.setName("label5").setDescription("The fifth button label").setRequired(false),
        ),
    )
    // Subcommand to view the current reaction role setups for this server.
    .addSubcommand((sub) => sub.setName("view").setDescription("View reaction role setups for this server")),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        const buttons = roles.map((role, i) =>
          new ButtonBuilder().setCustomId(customIds[i]).setLabel(labels[i]).setStyle(ButtonStyle.Primary),
        )
        ...
        for (const channelId in grouped) {
          ...
          if (description.length > 2048) {
            ...
            for (const line of lines) {
              if ((chunk + line + "\n").length > 2048) {
                embeds.push(
                  new EmbedBuilder().setColor("#FF00FF").setTitle("Reaction Role Setup").setDescription(chunk),
                )
                chunk = line + "\n"
              } else {
                chunk += line + "\n"
            ...
            embeds.push(
              new EmbedBuilder().setColor("#FF00FF").setTitle("Reaction Role Setup").setDescription(description),
            )
          }
        }
