  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option.setName("channelid").setDescription("The ID of the welcome channel").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("status").setDescription("Enable or disable the welcome channel").setRequired(true),
        ),
    )
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        if (!serverId || !channelId || status === null) {
          return interaction.reply({
            content: "Invalid input. Please provide valid server ID, channel ID, and status.",
            flags: 64,
          })
        }
        ...
        return interaction.reply({
          content: `Welcome channel updated successfully for server ID ${serverId}.`,
          flags: 64,
        })
