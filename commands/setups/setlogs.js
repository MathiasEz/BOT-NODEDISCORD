    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addChannelOption((option) =>
          option.setName("channel").setDescription("The channel to log the event in.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addChannelOption((option) =>
          option.setName("channel").setDescription("The channel to log all events in.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) => subcommand.setName("view").setDescription("View all configured logging channels.")),
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "event") {
        ...
        await logsCollection.updateOne({ guildId, eventType }, { $set: { channelId: channel.id } }, { upsert: true })
      ...
      if (subcommand === "all") {
        ...
        await Promise.all(
          eventTypes.map((eventType) =>
            logsCollection.updateOne({ guildId, eventType }, { $set: { channelId: channel.id } }, { upsert: true }),
          ),
        )
      ...
      if (subcommand === "view") {
        ...
        const embed = new EmbedBuilder().setTitle("Configured Logging Channels").setColor("#00FFFF")
