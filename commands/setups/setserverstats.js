...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addBooleanOption((option) =>
          option.setName("active").setDescription("Enable or disable this stat").setRequired(true),
        )
        ...
        .addStringOption((option) =>
          option.setName("name").setDescription("Custom name (use {count})").setRequired(false),
        ),
    )

    .addSubcommand((sub) => sub.setName("view").setDescription("View the current server stats setup"))
    ...
    .addSubcommand((sub) => sub.setName("clear").setDescription("Delete all server stats setups")),
  ...
  async execute(interaction) {
    ...
    if (subcommand === "setup") {
      ...
      const typesToCreate =
        type === "all" ? ["members", "bots", "textchannels", "voicechannels", "categories", "roles"] : [type]
      ...
      const statsList = stats
        .map(
          (s, i) => `\`${i + 1}\` | **${s.type}** → <#${s.channelId || "N/A"}> | **Active:** ${s.active ? "✅" : "❌"}`,
        )
        .join("\n")
