    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option.setName("channelid").setDescription("The ID of the leveling logs channel").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("enabled").setDescription("Enable or disable leveling").setRequired(true),
        ),
    )
    .addSubcommand((sub) => sub.setName("view").setDescription("View the current leveling configuration")),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
          return interaction.reply({
            embeds: [
              new EmbedBuilder().setColor("#ff0000").setDescription("You do not have permission to use this command."),
            ],
            flags: 64,
          })
        }
        ...
        if (!channel) {
          return interaction.reply({
            content: "Invalid channel ID. Please provide a valid channel ID from this server.",
            flags: 64,
          })
        }
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `Leveling has been **${enabled ? "enabled" : "disabled"}**.\nLogs will be sent to <#${channelId}>.`,
            )
          ...
          return interaction.reply({
            embeds: [
              new EmbedBuilder()
                .setColor("#ff0000")
                .setDescription("An error occurred while setting the leveling settings."),
            ],
            flags: 64,
          })
        }
        ...
        try {
          ...
          if (!configData) {
            return interaction.reply({
              embeds: [
                new EmbedBuilder()
                  .setColor("#ff0000")
                  .setDescription("No leveling configuration found for this server."),
              ],
              flags: 64,
            })
          }
          ...
          return interaction.reply({
            embeds: [
              new EmbedBuilder()
                .setColor("#ff0000")
                .setDescription("An error occurred while fetching the leveling settings."),
            ],
            flags: 64,
          })
        }
