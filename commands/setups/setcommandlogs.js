...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    // Subcommand: view
    .addSubcommand((subcommand) =>
      subcommand.setName("view").setDescription("View the current command logging configuration."),
    ),
  async execute(interaction) {
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      // If using the "view" subcommand
        ...
        const embed = new EmbedBuilder().setColor("#3498db").setTitle("Command Logging Configuration").setTimestamp()

        if (!config || config.enabled === false) {
          embed.setDescription("Command logging is currently **disabled** for this server.")
        } else {
          embed
            .setDescription("Command logging is **enabled** for this server.")
            .addFields(
