  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option.setName("voicechannelid").setDescription("The ID of the main voice channel").setRequired(true),
        )
        .addStringOption((option) =>
          option.setName("managerchannelid").setDescription("The ID of the manager channel").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("status").setDescription("The status of the voice channel setup").setRequired(true),
        ),
    )
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        if (!serverId || !voiceChannelId || !managerChannelId || status === null) {
          return interaction.reply({
            content: "Invalid input. Please provide valid server ID, voice channel ID, manager channel ID, and status.",
            flags: 64,
          })
        }
        ...
        return interaction.reply({
          content: `Voice channel setup updated successfully for server ID ${serverId}.`,
          flags: 64,
        })
