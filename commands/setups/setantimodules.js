...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addBooleanOption((option) =>
          option.setName("antispam").setDescription("Enable or disable anti-spam module").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("antilink").setDescription("Enable or disable anti-link module").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("antinuke").setDescription("Enable or disable anti-nuke module").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("antiraid").setDescription("Enable or disable anti-raid module").setRequired(true),
        )
        .addStringOption((option) =>
          option.setName("logchannelid").setDescription("The ID of the log channel").setRequired(true),
        )
        ...
        .addStringOption((option) =>
          option.setName("adminid2").setDescription("The second admin ID").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("messagecount").setDescription("Max messages allowed in time window"),
        )
        ...
        .addIntegerOption((option) =>
          option.setName("duration").setDescription("Timeout duration in ms if action is timeout"),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("channeldeletelimit").setDescription("Max channel deletions allowed"),
        )
        .addIntegerOption((option) =>
          option.setName("channeldeletetime").setDescription("Time window for channel deletions in ms"),
        )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("joinlimit").setDescription("Max members allowed to join in time window"),
        )
  ...
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      const guildConfig = await antisetupCollection.findOne({ serverId })
      ...
      if (subcommand === "initial") {
        ...
        const ownerIds = [interaction.options.getString("ownerid1"), interaction.options.getString("ownerid2")]
        const adminIds = [interaction.options.getString("adminid1"), interaction.options.getString("adminid2")]
        ...
        if (ownerIds.includes(memberId) || adminIds.includes(memberId)) {
          ...
          try {
            await antisetupCollection.updateOne({ serverId }, { $set: config }, { upsert: true })
            interaction.reply({
              content: `Anti-modules, log channel, owners, and admins updated successfully for server ID ${serverId}.`,
              flags: 64,
            })
          } catch (err) {
          ...
          interaction.reply({
            content:
              "You do not have permission to use this command. Only owners or admins can configure anti-modules.",
            flags: 64,
          })
        }
      } else {
        if (!guildConfig) {
          return interaction.reply({
            content: "This server is not configured. Please run the initial setup first.",
            flags: 64,
          })
        }
        ...
        try {
          await antisetupCollection.updateOne({ serverId }, { $set: guildConfig }, { upsert: true })
