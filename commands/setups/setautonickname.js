    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option.setName("prefix").setDescription("The prefix to add to nicknames.").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("status").setDescription("Enable or disable the auto nickname changer.").setRequired(false),
        ),
    )
    // "view" subcommand to view the current configuration
    .addSubcommand((sub) =>
      sub.setName("view").setDescription("View the current auto nickname changer configuration."),
    ),
  ...
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        await nicknameConfigs.updateOne({ guildId }, { $set: { nicknamePrefix: prefix, status } }, { upsert: true })
