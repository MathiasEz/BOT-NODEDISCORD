  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option.setName("channelid").setDescription("The ID of the ticket channel").setRequired(true),
        )
        .addStringOption((option) =>
          option.setName("adminroleid").setDescription("The ID of the admin role for tickets").setRequired(true),
        )
        .addBooleanOption((option) =>
          option.setName("status").setDescription("Enable or disable the ticket system").setRequired(true),
        ),
    )
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        if (!serverId || !channelId || !adminRoleId || status === null) {
          return interaction.reply({
            content: "Invalid input. Please provide valid server ID, channel ID, admin role ID, and status.",
            flags: 64,
          })
        }
        ...
        return interaction.reply({
          content: `Ticket channel updated successfully for server ID ${serverId}.`,
          flags: 64,
        })
