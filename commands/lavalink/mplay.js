...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("query").setDescription("Enter the song name or URL.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("nowplaying").setDescription("Get information about the currently playing song."),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("loop")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("track").setDescription("Track number to remove.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) => subcommand.setName("viewmyplaylists").setDescription("View your saved playlists."))
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) => subcommand.setName("allplaylists").setDescription("View all public playlists."))
    .addSubcommand((subcommand) =>
      subcommand
        .setName("deletesong")
        ...
        .addIntegerOption((option) =>
          option.setName("index").setDescription("Song index to remove.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("song").setDescription("Enter song name or URL.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("level").setDescription("Volume level (0-100).").setRequired(true),
        ),
    ),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "play") {
        ...
        if (!permissions.has(PermissionFlagsBits.Connect) || !permissions.has(PermissionFlagsBits.Speak)) {
          return interaction.editReply({
            content: "❌ I need permission to connect & speak in the voice channel.",
            flags: 64,
          })
        }
      ...
      if (subcommand === "pause") {
        ...
        await interaction.editReply({
          content: `🎶 Queue: ${queue.map((track, i) => `${i + 1}. **${track.info.title}**`).join("\n")}`,
        })
      ...
      // 🎶 PLAY PLAYLIST
        ...
        if (playlist.visibility === "private" && playlist.owner !== userId) {
          return interaction.editReply({
            content: `❌ You do not have permission to play this private playlist.`,
            ephemeral: true,
          })
        }
        ...
        if (!channel) {
          return interaction.editReply({
            content: "❌ You must be in a voice channel to play a playlist.",
            ephemeral: true,
          })
        }
        ...
        if (addedTracks === 0) {
          return interaction.editReply({
            content: `❌ Could not resolve any valid tracks from playlist **${name}**.`,
            ephemeral: true,
          })
        }
      ...
      // 🌍 VIEW ALL PUBLIC PLAYLISTS
        ...
        if (playlist.visibility === "private" && playlist.owner !== userId) {
          return interaction.editReply({
            content: `❌ You do not have permission to add songs to this private playlist.`,
            ephemeral: true,
          })
        }

        // Add song to the playlist
        await playlistCollection.updateOne({ name: playlistName }, { $push: { songs: songInput } })
        ...
        if (playlist.visibility === "private" && playlist.owner !== userId) {
          return interaction.editReply({
            content: `❌ You do not have permission to view this private playlist.`,
            ephemeral: true,
          })
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({
            text: playlist.songs.length > 10 ? `+ ${playlist.songs.length - 10} more songs...` : "End of playlist",
          })
