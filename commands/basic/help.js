...
module.exports = {
  data: new SlashCommandBuilder().setName("help").setDescription("Displays the command list and bot information"),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
                    ...
                    if (dataJSON.options && Array.isArray(dataJSON.options)) {
                      for (const option of dataJSON.options) {
                        if (option.type === 1) {
                          ...
                          const groupSubs = option.options.filter((opt) => opt.type === 1).map((opt) => opt.name)
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.isStringSelectMenu()) {
          currentPage = Number.parseInt(i.values[0])
        } else if (i.isButton()) {
          switch (i.customId) {
            case "previous":
