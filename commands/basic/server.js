...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("info").setDescription("Show detailed server information with pagination."),
    )
    .addSubcommand((subcommand) => subcommand.setName("icon").setDescription("Show the server icon."))
    .addSubcommand((subcommand) => subcommand.setName("banner").setDescription("Show the server banner.")),
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "info") {
        try {
          ...
          const embeds = [
            ...
            new EmbedBuilder()
              ...
              .setTitle("🎭 Roles")
              .setDescription(
                roles.size > 0 ? roles.map((role) => `<@&${role.id}>`).join(", ") : "No roles available.",
              ),
        ...
        if (!bannerURL) {
          const embed = new EmbedBuilder().setColor("#FF0000").setDescription(lang.serverNoBanner)
