...
async function endQuiz(interaction, channelId, reason) {
  ...
  if (quizData) {
    ...
    if (channel) {
      const endEmbed = new EmbedBuilder().setTitle("📚 Math Quiz Ended! 🧠").setDescription(reason).setColor(0xff0000)
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => subcommand.setName("start").setDescription("Start a math quiz in this channel.")),
        ...
        const startNextQuestion = async () => {
          if (quizData.currentQuestionIndex >= quizData.questions.length) {
            await endQuiz(
              interaction,
              channelId,
              `🎉 Quiz completed! You scored ${quizData.correctAnswers}/${quizData.questions.length}.`,
            )
            return
          }
          ...
          const quizEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `**Question ${quizData.currentQuestionIndex + 1}/5:** What is ${question}? Respond with \`!<your answer>\``,
            )
            .setColor(0x0099ff)
          ...
          quizData.collector.on("collect", async (response) => {
            const userAnswer = Number.parseInt(response.content.slice(1).trim(), 10)
          ...
          quizData.collector.on("end", async (collected) => {
            if (collected.size === 0) {
              ...
              if (quizData.timeoutCount >= 3) {
                ...
                const timeoutMessage = await interaction.channel.send(
                  `⏳ Time is up! Moving to the ${quizData.currentQuestionIndex + 1}/5 question. ⏭️`,
                )
