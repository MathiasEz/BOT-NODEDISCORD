  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        ...
        .setRequired(true)
        .addChoices({ name: "VS Bot", value: "vsbot" }, { name: "VS User", value: "vsuser" }),
    )
    .addUserOption((option) =>
      option.setName("opponent").setDescription("The user you want to play against.").setRequired(false),
    ),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (mode === "vsbot" && opponent.id !== interaction.client.user.id) {
        return interaction.reply({
          content: "🚫 You cannot mention an opponent when playing against the bot.",
          ephemeral: true,
        })
      }
      ...
      if (mode === "vsuser" && opponent.id === interaction.client.user.id) {
        return interaction.reply({
          content: "🚫 You cannot play against the bot in VS User mode. Please choose a different opponent.",
          ephemeral: true,
        })
      }
      ...
      if (activeGames[serverId]) {
        return interaction.reply({
          content:
            "🚫 A game is already in progress in this server! Please wait for the current game to finish before starting a new one.",
          ephemeral: true,
        })
      }

      const startingUserId =
        mode === "vsbot" ? interaction.user.id : Math.random() < 0.5 ? interaction.user.id : opponent.id
      ...
      const filter = (m) =>
        [interaction.user.id, opponent.id].includes(m.author.id) && !isNaN(m.content) && m.content.trim() !== ""
      ...
      const takeBotTurn = async () => {
        ...
        if (activeGames[serverId].score >= targetScore) {
          ...
          const winEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `Congratulations! 🎉 The Bot reached ${targetScore} first!\n\nFinal score: ${activeGames[serverId].score}`,
            )
            .setColor("#FFD700")
            .setTimestamp()
          ...
          const turnEmbed = new EmbedBuilder()
            .setTitle("🔄 Next Turn")
            .setDescription(
              `It's <@${activeGames[serverId].currentTurnUserId}>'s turn! Current score: ${activeGames[serverId].score}`,
            )
            .setColor("#00FF00")
            .setFooter({ text: "Type a number to play!" })
            .setTimestamp()
      ...
      const handleUserTurn = async (m) => {
        ...
        const userChoice = Number.parseInt(m.content)
        ...
        if (userId !== activeGames[serverId].currentTurnUserId) {
          const replyMessage = await m.reply({
            content: `🚫 It's not your turn! Please wait for <@${activeGames[serverId].currentTurnUserId}>'s turn.`,
            ephemeral: true,
          })
        ...
        if (userChoice < minNumber || userChoice > maxNumber) {
          const replyMessage = await m.reply({
            content: `🚫 You can only add 1 or 2 to the current score. Please choose a valid number between ${minNumber} and ${maxNumber}.`,
            ephemeral: true,
          })
          setTimeout(() => replyMessage.delete(), 10000)
          return
        ...
        if (activeGames[serverId].score >= targetScore) {
          ...
          const winEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `🎉 ${m.author.username} reached ${targetScore} first and won the game!\n\nFinal score: ${activeGames[serverId].score}`,
            )
            .setColor("#FFD700")
            .setTimestamp()
          ...
          activeGames[serverId].currentTurnUserId =
            activeGames[serverId].currentTurnUserId === interaction.user.id ? opponent.id : interaction.user.id
          ...
          if (
            activeGames[serverId].mode === "vsbot" &&
            activeGames[serverId].currentTurnUserId === interaction.client.user.id
          ) {
            await takeBotTurn()
          }
        }
