...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) => option.setName("min").setDescription("Minimum value (inclusive)").setRequired(true))
    .addIntegerOption((option) => option.setName("max").setDescription("Maximum value (inclusive)").setRequired(true)),
  ...
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          lang.randomnumDescription.replace("{min}", min).replace("{max}", max).replace("{randomNumber}", randomNumber),
        )
        .setTimestamp()
