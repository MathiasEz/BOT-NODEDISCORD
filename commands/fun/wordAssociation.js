...
module.exports = {
  data: new SlashCommandBuilder().setName("wordassociation").setDescription(lang.wordAssociationDescription), // Use localized description
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (missingPermissions.length > 0) {
        ...
        return interaction.reply({
          content: `${lang.missingPermissionsMessage} ${permissionNames}. ${lang.adjustPermissions}`,
          ephemeral: true,
        })
      }

      const existingChannel = interaction.guild.channels.cache.find(
        (channel) => channel.name === `word-association-${commandUser.username.toLowerCase()}`,
      )
      if (existingChannel) {
        return interaction.reply({
          content: `${lang.existingGameMessage} ${existingChannel}. ${lang.finishCurrentGame}`,
          ephemeral: true,
        })
      }
      ...
      if (!interaction.replied) {
        await interaction.reply({
          content: `${lang.gameStartedMessage} ${tempChannel}. ${lang.goodLuck}`,
          ephemeral: true,
        })
      }
      ...
      collector.on("collect", async (message) => {
        ...
        if (userWord.startsWith("!")) {
          if (userWord === currentWord) {
            ...
            await tempChannel.send(
              `${lang.goodChoiceMessage} ${message.author}! ${lang.wordUpdatedMessage}\n${lang.hintMessage}: ${wordHints[currentWord]}`,
            )
            try {
