    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName("user").setDescription("Mention a user to check their Epic Games name.").setRequired(false),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("epicname").setDescription("Your Epic Games name.").setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("removename").setDescription("Remove your Epic Games account name."),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName("user").setDescription("Mention a user to check their stats.").setRequired(false),
        )
        .addStringOption((option) =>
          option.setName("epicname").setDescription("Enter an Epic Games name to check stats.").setRequired(false),
        ),
    ),
      ...
      if (subcommand === "account") {
        ...
        if (epic) {
          ...
          embed.setDescription(
            `${user.username} has not set their Epic Games name yet. Use \`/epic setname\` to set it.`,
          )
        }
        ...
        if (!epicName) {
          ...
          if (!epicName) {
            return interaction.editReply(
              `${user.username} has not set their Epic Games name. Use \`/epic setname\` to set it.`,
            )
          }
        }
        ...
        try {
          ...
          if (data.status !== 200 || !data.data) {
            return interaction.editReply(
              "❌ Error fetching Fortnite stats. Please ensure the Epic Games name is correct.",
            )
          }
