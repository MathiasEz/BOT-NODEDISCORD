...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => subcommand.setName("leaderboard").setDescription("Displays the XP leaderboard."))
    .addSubcommand((subcommand) =>
      subcommand
        .setName("rank")
        .setDescription("Show your rank or another user's rank.")
        .addUserOption((option) =>
          option.setName("user").setDescription("The user to check the rank for.").setRequired(false),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("weeklyxp").setDescription("Shows how much XP you earned this week."),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("xpforlevel").setDescription("Shows how much XP is needed for the next level."),
    ),
  ...
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "givexp") {
        ...
        const leaderboardEntries = leaderboardData.map(
          (user, index) => `**${index + 1}.** <@${user.userId}> - Level **${user.level}**, XP: **${user.xp}**`,
        )
        ...
        return interaction.editReply(
          `📅 **${interaction.user.username}** earned **${userData.weeklyXp} XP** this week.`,
        )
        ...
        return interaction.editReply(
          `✨ **${interaction.user.username}** needs **${xpNeeded} XP** to reach the next level.`,
        )
      }
    } else {
      const embed = new EmbedBuilder()
