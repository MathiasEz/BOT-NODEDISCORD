    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("trigger").setDescription("The trigger word or phrase.").setRequired(true),
        )
        .addStringOption((option) =>
          option.setName("channels").setDescription('Channel(s) (comma-separated IDs or "all").').setRequired(true),
        )
        .addStringOption((option) =>
          option
            .setName("match")
        ...
        .addStringOption((option) =>
          option.setName("text_response").setDescription("Non-embed response (optional).").setRequired(false),
        )
        .addBooleanOption((option) =>
          option.setName("use_embed").setDescription("Use embed response? (true = embed, false = text)"),
        )
        .addStringOption((option) =>
          option.setName("embed_title").setDescription("Embed title (required for embed).").setRequired(false),
        )
        .addStringOption((option) =>
          option
            .setName("embed_color")
        ...
        .addStringOption((option) =>
          option.setName("embed_footer").setDescription("Embed footer text (optional).").setRequired(false),
        )
        .addStringOption((option) =>
          option.setName("embed_image").setDescription("Embed image URL (optional).").setRequired(false),
        )
        .addStringOption((option) =>
          option.setName("embed_thumbnail").setDescription("Embed thumbnail URL (optional).").setRequired(false),
        )
        .addBooleanOption((option) => option.setName("status").setDescription("Enable or disable the AutoResponder.")),
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option.setName("index").setDescription("The index number of the AutoResponder.").setRequired(true),
        ),
    )

    .addSubcommand((subcommand) => subcommand.setName("view").setDescription("View all AutoResponders you created.")),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        const list = autoResponders
          .map(
            (res, i) =>
              `**#${i + 1}** | Trigger: \`${res.trigger}\` | Channels: ${res.channels.join(", ")} | Status: ${res.status ? "✅ Enabled" : "❌ Disabled"}`,
          )
          .join("\n")
