  ...
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .addStringOption((option) =>
        option.setName("description").setDescription("Giveaway description").setRequired(true),
      )
      .addStringOption((option) => option.setName("prize").setDescription("Prize description").setRequired(true))
      .addStringOption((option) =>
        option.setName("duration").setDescription("Giveaway duration (e.g., 1m, 24h, 3d, 1 week)").setRequired(true),
      )
      .addIntegerOption((option) => option.setName("winners").setDescription("Number of winners").setRequired(false))
      .addChannelOption((option) =>
        option.setName("channel").setDescription("Channel for the giveaway").setRequired(false),
      )
      .addRoleOption((option) =>
        option.setName("roles").setDescription("Role requirement to enter").setRequired(false),
      ),
  )
...
module.exports = {
  ...
  async execute(interaction) {
    if (!interaction.member.permissions.has(PermissionFlagsBits.ModerateMembers)) {
      const embed = new EmbedBuilder().setColor("#ff0000").setDescription(lang.giveawayNoPermissions)
    ...
    const message = await channel.send({
      ...
      components: [createGiveawayButtons({ entries: [] })],
    })
