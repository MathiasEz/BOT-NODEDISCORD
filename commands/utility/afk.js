  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) => option.setName("reason").setDescription("Reason for going AFK.").setRequired(true))
        .addStringOption((option) =>
          option.setName("duration").setDescription("Duration of AFK (e.g., 10m, 1h).").setRequired(false),
        ),
    )
    .addSubcommand((subcommand) => subcommand.setName("list").setDescription("Show a list of AFK users in the server."))
    .addSubcommand((subcommand) => subcommand.setName("remove").setDescription("Remove your AFK status.")),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "set") {
        ...
        await interaction.user
          .send(`You are now AFK: ${reason}${duration ? ` for ${durationStr}` : ""}`)
          .catch(console.error)
        ...
        for (const chunk of afkChunks) {
          const embed = new EmbedBuilder().setTitle("AFK Users").setColor(0xffcc00).setTimestamp()
