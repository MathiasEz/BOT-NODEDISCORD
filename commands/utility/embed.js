    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("fields")
            .setDescription('Fields (JSON format: [{"name":"Name","value":"Value"}]).')
            .setRequired(false),
        )
        .addStringOption((option) => option.setName("image").setDescription("Image URL.").setRequired(false))
        .addBooleanOption((option) =>
          option.setName("timestamp").setDescription("Include timestamp?").setRequired(false),
        )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addChannelOption((option) =>
          option.setName("channel").setDescription("Channel to send the embed.").setRequired(true),
        )
        .addStringOption((option) =>
          option.setName("embed_name").setDescription("Saved embed name (leave empty for new).").setRequired(false),
        )
        .addStringOption((option) => option.setName("title").setDescription("Embed title.").setRequired(false))
        .addStringOption((option) =>
          option.setName("description").setDescription("Embed description.").setRequired(false),
        ...
        .addStringOption((option) =>
          option.setName("non_embed_content").setDescription("Text before the embed.").setRequired(false),
        )
        .addIntegerOption((option) =>
          option.setName("timer").setDescription("Delay in seconds before sending.").setRequired(false),
        ),
    ),
    ...
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (subcommand === "save") {
        ...
        try {
          ...
          return interaction.reply({
            content: '❌ Invalid fields format. Use JSON like `[{"name":"Field","value":"Value"}]`.',
            ephemeral: true,
          })
        }
      ...
      if (subcommand === "view") {
        ...
        if (!embedName) {
          ...
          const embed = new EmbedBuilder().setTitle("📜 Saved Embeds").setDescription(embedList).setColor("#3498db")
      ...
      // Delete Embed
        ...
        if (!embed)
          return interaction.reply({ content: `❌ No embed found with the name **${name}**.`, ephemeral: true })
        ...
        // If creating a new embed
          ...
          embed = new EmbedBuilder()
            .setColor(Number.parseInt(color.replace("#", ""), 16)) // Convert hex to integer
